...
/**
 * Implements a School session.
 * 
 * @author Robert M. Preston
 * 
 */
class SchoolSession extends Session3270 implements Seconds {
    private UserInterface myUi;

    /**
     * Creates a School session.
     * 
     ...
     */
    SchoolSession(String name, UserInterface ui) throws JagacyException {
        super(name, "teague-tammvs1.tamu.edu", "IBM-3279-2-E");
        myUi = ui;
    }
    ...
    protected boolean logon() throws JagacyException {
        if (!myUi.waitForPosition(0, 1, "SYSTEM", THIRTY_SECONDS)) {
            return false;
        }
    ...
    protected void logoff() throws JagacyException {
        if (myUi.waitForPosition(0, 1, "SYSTEM", 0)) {
            writeString("exit");
            writeKey(Key.ENTER);
            try {
                myUi.waitForChange(FIVE_SECONDS);
            } catch (JagacyException e) {
                if (e.getError() != JagacyException.CONDUIT_DISCONNECT_ERROR) {
                    throw e;
                }
            }
        ...
        if (myUi.waitForPosition(0, 2, "Welcome", 0)) {
            writeKey(Key.TAB);
            writeString("Q");
            writeKey(Key.ENTER);
            try {
...
public class MyEmulator extends AbstractSwing {
    ...
    protected AbstractSession createSession(String name) throws JagacyException {
        mySession = new SchoolSession(name, this);
        return mySession;
    }
    ...
    protected JMenu addMenu(JMenu menu) {
        ...
        if (menu.getActionCommand().equals(I18n.getText("gui.menu.edit"))) {
            newMenu = createMenu(I18n.getText("myemulator.options"), KeyEvent.VK_O);
            newMenu.add(createMenuItem(I18n.getText("myemulator.look_and_feel"),
    ...
    protected JButton addButton(JButton button) {
        ...
        if (button.getActionCommand().equals("Paste")) {
            newButton = createButton(MyEmulator.class, "images/duke.gif",
                "Look and Feel", I18n.getText("myemulator.tip"));
        }
        return newButton;
    }
    ...
    private void changeLookAndFeel() {
        myIsCancelled = false;
        final JDialog dialog = new JDialog(this, I18n.getText("myemulator.look_and_feel"), true);
